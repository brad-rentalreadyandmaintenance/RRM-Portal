...
# 4. AUTHENTICATION (URL PERSISTENCE)
params = st.query_params
if "user" in params:
    st.session_state.update({
        "auth": True, 
        "user": params["user"], 
        "role": params.get("role", "Tech")
    })
else:
...
if view == "Admin":
    ...
    with t1:
        with st.form("disp"):
            t = st.selectbox("Tech", ud['User'])
            cl = st.selectbox("Client", cd['Client'])
    ...
    with t2:
        ...
        with c2:
            ...
            with st.form("cf"):
                cn = st.text_input("Name", value=c_target if c_target else "")
    ...
    with t3:
        ...
        f_tech = st.multiselect("Tech", ud['User'].unique(), default=ud['User'].unique())
        ...
        st.metric("Total Hours", f"{f_ld['Duration'].apply(to_h).sum():.2f}")
        st.dataframe(f_ld, use_container_width=True)
        
        if not f_ld.empty:
            sel = st.selectbox("View Photos", f_ld.index)
            det = f_ld.loc[sel]
...
# 7. FIELD PORTAL
    ...
    if 'job' not in st.session_state:
        ...
        for i, r in td[(td['Tech']==st.session_state.user)&(td['Status']=="Pending")].iterrows():
            if st.button(f"Clock In: {r['Client']} (Unit {r['Unit']})"):
                st.session_state.job = {"c": r['Client'], "u": r['Unit'], "m": "D"}
                st.session_state.start = get_mst()
                st.rerun()
        
        st.divider()
        is_new = st.checkbox("Manual/New Client")
        m_c = st.text_input("Client") if is_new else st.selectbox("Existing", ["--"] + cd['Client'].tolist())
